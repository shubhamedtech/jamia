  ...
  <div class="content-wrapper">
    <div class="content-header sty-one">
      <div class="d-flex align-items-center justify-content-between">
        ...
        ?>
        <button class="btn btn-link p-2" aria-label="" title="" id = "download_button" data-toggle="tooltip" data-original-title="Download"><i class="fa fa-download fa-lg"></i></button>
      </div>
    </div>
    ...
    <script type="text/javascript">
      ...
      $("#download_button").on('click', function(){
        var course_id = $('#courses').val();
        var session_id = $("#session").val();
        var search = $('#subject-search-table').val();
        $.ajax({
          url : '/app/datesheets/export',
          type : 'post',
          data : {'course_id' : course_id , 'session_id' : session_id , 'search' : search},
          success : function(data) {
            var blob=new Blob([data]);
            var link=document.createElement('a');
            link.href=window.URL.createObjectURL(blob);
            link.download="date-sheets.csv";
            link.click();
          }
        });
      });
