  ...
  <div class="content-wrapper">
    <div class="content-header sty-one">
      <div class="d-flex align-items-center justify-content-between">
        ...
        ?>
        <div>
          <a href="/app/payments/export?type=2" target="_blank" class="btn btn-link p-2" aria-label="" title="" data-toggle="tooltip" data-original-title="Download" onclick="exportData()"> <i class="fa fa-download fa-lg"></i></a>
          <?php if (isset($_SESSION['gateway'])) { ?>
            <button class="btn btn-link" aria-label="" title="" data-toggle="tooltip" data-original-title="Pay Now" onclick="add('<?php echo $_SESSION['gateway'] == 1 ? 'easebuzz' : '' ?>', 'md')"> <i class="fa fa-plus-circle"></i></button>
          <?php } ?>
        </div>
      </div>
    </div>
            ...
            "render": function(data, type, row) {
              if (['Center', 'Sub-Center'].includes(role)) {
                if (row.File.length == 0) {
                  return '<center><i class="fa fa-upload cursor-pointer" onclick="uploadReceipt(' + data + ')" data-toggle="tooltip" data-placement="top" title="Upload Receipt"></i></center>';
                } else {
