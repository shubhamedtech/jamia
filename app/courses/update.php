...
ini_set('display_errors',1);
  if(isset($_POST['name']) && isset($_POST['course_type']) && isset($_POST['university_id']) && isset($_POST['id']) && isset($_POST['department'])){
    ...
    $add = $conn->query("UPDATE `Courses` SET `Name` = '$name', `Short_Name` = '$short_name', `Registration_Fee` = $registration_fee, `Exam_Fee` = $exam_fee, `Course_Type_ID` = $course_type, `Department_ID` = $department_id WHERE ID = $id");
    if($add){
