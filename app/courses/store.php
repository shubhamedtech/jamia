<?php
  if(isset($_POST['name']) && isset($_POST['course_type']) && isset($_POST['university_id']) && isset($_POST['department'])){
    ...
    $exam_fee = intval($_POST['exam_fee']) ? intval($_POST['exam_fee']) : 0;
    $registration_fee = intval($_POST['registration_fee']) intval($_POST['registration_fee']) : 0;
    ...
    $add = $conn->query("INSERT INTO `Courses`(`Name`, `Short_Name`, `Exam_Fee`, `Registration_Fee`, `Course_Type_ID`, `University_ID`, `Department_ID`) VALUES ('$name', '$short_name',  $exam_fee,  $registration_fee, $course_type, $university_id, $department_id)");
    if($add){
