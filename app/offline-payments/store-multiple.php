...
if (isset($_POST['amount']) && isset($_POST['transaction_id']) && isset($_POST['ids'])) {
  ...
  $amount_val = mysqli_real_escape_string($conn, $_POST['amount']);
  ...
  foreach ($ids as $id) {
    ...
    $amount = balanceAmount($conn, $id, $duration);
    ...
    $add = $conn->query("INSERT INTO Invoices (`User_ID`, `Student_ID`, `Duration`, `University_ID`, `Invoice_No`, `Amount`) VALUES (" . $_SESSION['ID'] . ", $id, $duration, " . $_SESSION['university_id'] . ", '$transaction_id', $amount)");
   // $conn->query("UPDATE Students SET Process_By_Center = now() WHERE ID = $id ");
  }
  if ($add) {
    $add = $conn->query("INSERT INTO Payments (Type, Transaction_Date, Transaction_ID, Gateway_ID, Bank, Amount, Payment_Mode, Added_By, File, University_ID) VALUES (1, '$transaction_date', '$transaction_id', '$gateway_id', '$bank_name', '$amount_val', '$payment_type', " . $_SESSION['ID'] . ", '$file', " . $_SESSION['university_id'] . ")");
    if ($add) {
