...
if (isset($_POST['amount']) && isset($_POST['transaction_id'])) {
  ...
  if (!empty($student_id)) {
    ...
    $added_by = $conn->query("SELECT `Added_For`,Duration FROM `Students` WHERE ID = $student_id");
    $added_by = $added_by->fetch_assoc();
    $duration = $added_by['Duration'];
    $added_by = $added_by['Added_For'];
   
  }
  $add_invoice = $conn->query("INSERT INTO Invoices (`User_ID`, `Student_ID`, `Duration`, `University_ID`, `Invoice_No`, `Amount`) VALUES (" . $_SESSION['ID'] . ", $student_id, $duration, " . $_SESSION['university_id'] . ", '$transaction_id', $amount)");
  $conn->query("UPDATE Students SET Process_By_Center = now() WHERE ID = $student_id ");
  $add = $conn->query("INSERT INTO Payments (Type, Transaction_Date, Transaction_ID, Gateway_ID, Bank, Amount, Payment_Mode, Added_By, File, University_ID ) VALUES (1, '$transaction_date', '$transaction_id', '$gateway_id', '$bank_name', '$amount', '$payment_type', " . $added_by . ", '$file', " . $_SESSION['university_id'] . ")");
  if ($add) {
