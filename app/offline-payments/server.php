...
if ($searchValue != '') {
  ...
  if (strcasecmp($searchValue, 'pending') == 0) {
    ...
    $searchQuery = " AND (CONCAT(TRIM(CONCAT(Students.First_Name, ' ', Students.Middle_Name, ' ', Students.Last_Name)), ' (', IF(Students.Unique_ID='' OR Students.Unique_ID IS NULL, RIGHT(CONCAT('000000', Students.ID), 6), Students.Unique_ID), ')') LIKE '%" . $searchValue . "%' OR Payments.Bank like '%".$searchValue."%' OR Transaction_ID like '%" . $searchValue . "%' OR Gateway_ID like '%" . $searchValue . "%' OR Amount like '%" . $searchValue . "%' OR Payment_Mode like '%" . $searchValue . "%' OR Universities.Short_Name like '%" . $searchValue . "%' OR Universities.Name like '%" . $searchValue . "%' OR Universities.Vertical like '%" . $searchValue . "%')";
  }
}
