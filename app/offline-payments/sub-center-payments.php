...
// Added_For
$sub_sub_center_id = array();
$sub_centers = $conn->query("SELECT Center_SubCenter.Sub_Center FROM Users LEFT JOIN Center_SubCenter ON Users.ID = Center_SubCenter.Center WHERE Center_SubCenter.Center = ".$_SESSION['ID']."");
while ($sub_center = mysqli_fetch_assoc($sub_centers)) {
  $sub_sub_center_id[] = $sub_center['Sub_Center'];
}
$sub_center_id = implode(',', $sub_sub_center_id);
...
$result_record = "SELECT CONCAT(TRIM(CONCAT(Students.First_Name, ' ', Students.Middle_Name, ' ', Students.Last_Name)), ' (', IF(Students.Unique_ID='' OR Students.Unique_ID IS NULL, RIGHT(CONCAT('000000', Students.ID), 6), Students.Unique_ID), ')') as Student_Name, Payments.ID, Payments.Transaction_Date, Payments.Transaction_ID, Payments.Gateway_ID, Payments.Amount, Payments.File, Payments.Payment_Mode, Payments.Bank, Students.Added_For, Payments.Status, CONCAT(Universities.Short_Name, ' (', Universities.Vertical, ')') as University FROM Students LEFT JOIN Universities ON Students.University_ID = Universities.ID LEFT JOIN Student_Ledgers ON Students.ID = Student_Ledgers.Student_ID LEFT JOIN Invoices ON Students.ID = Invoices.Student_ID LEFT JOIN Payments ON Invoices.Invoice_No = Payments.Transaction_ID WHERE Students.Added_For IN($sub_center_id) GROUP BY Students.ID";
...
while ($row = mysqli_fetch_assoc($empRecords)) {
  // Added_For
  if(isset($row['Added_For'])){
    $user = $conn->query("SELECT Center_SubCenter.Sub_Center, Users.ID, Code, Name FROM Users LEFT JOIN Center_SubCenter ON Users.ID = Center_SubCenter.Sub_Center WHERE Center_SubCenter.Sub_Center = ".$row['Added_For']." AND Center_SubCenter.Center = ".$_SESSION['ID']."");
  }
  ...
  if (!empty($user)) {
    // RM
    $rm = $conn->query("SELECT CONCAT(Users.Name, ' (', Users.Code, ')') as Name, Users.ID FROM Alloted_Center_To_Counsellor LEFT JOIN Users ON Alloted_Center_To_Counsellor.Counsellor_ID = Users.ID AND Alloted_Center_To_Counsellor.University_ID = " . $_SESSION['university_id'] . " WHERE Alloted_Center_To_Counsellor.Code = " . $user['Sub_Center'] . " AND Alloted_Center_To_Counsellor.University_ID = " . $_SESSION['university_id']);
    if ($rm->num_rows > 0) {
  ...
  $data[] = array(
    ...
    "Total_Amount" => empty($row['Amount']) ? '' : $row['Amount'],
    "Center_Amount" => empty($row['Amount']) ? '' : 2000,
