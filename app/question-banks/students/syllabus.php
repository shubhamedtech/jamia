                ...
                <?php if (in_array($_SESSION['Role'], ['Student'])) {
                  ...
                  <ul class="list-unstyled text-left">
                    <?php while ($unit = $units->fetch_assoc()) {
                      ...
                      $student_assessments = $conn->query("SELECT ID FROM Students_Assessments WHERE Assessment_ID =  $assessment_id AND Student_ID = ".$_SESSION['ID']." AND E_Book_ID = $E_books_id AND Unit_ID = " . $unit['ID'] . " AND Subject_ID = " . $unit['Syllabi_ID'] . "");
                    ?>
                      
                      <button class="btn btn-primary cursor-pointer"><?= $unit['Unit_Name'] ?></button>
                      
                      <?php if($student_assessments->num_rows > 0){ ?>
                        <button class="btn btn-success cursor-pointer" onclick="E_bookList(<?= $E_books_id ?>, <?= $E_books_unit_id ?>,<?= $E_books_subject_id ?>);"><?= $E_books->num_rows ?> <i class="fa fa-book mr-1"></i></button>
                        <span class="btn btn-sm btn-success cursor-pointer m-2" onclick="assessmentList(<?= $assessment_id ?>, <?= $E_books_id ?>, <?= $E_books_unit_id ?>, <?= $E_books_subject_id ?>);"><?= $assessments->num_rows ?> <i class="fa-pencil-square-o fa"></i></span>
                      <?php } else { ?>
                        <button class="btn btn-secondary cursor-pointer" onclick="E_bookList(<?=$E_books_id ?>, <?=$E_books_unit_id ?>,<?=$E_books_subject_id ?>);"><?= $E_books->num_rows ?> <i class="fa fa-book mr-1"></i></button>
                        <span class="btn btn-sm btn-warning cursor-pointer m-2" onclick="assessmentList(<?= $assessment_id ?>, <?= $E_books_id ?>, <?= $E_books_unit_id ?>, <?= $E_books_subject_id ?>);"><?= $assessments->num_rows ?> <i class="fa-pencil-square-o fa"></i></span>
                  <?php } }
                  } ?>
              </td>
            </tr>
