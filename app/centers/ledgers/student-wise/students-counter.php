<?php if (isset($_GET['id'])) {
  ...
  $stu_ids_arr = [];
  $stu_ids = '';
  $stu_id_sql = $conn->query("SELECT Student_ID FROM Invoices WHERE Invoices.User_ID IN($id)");
  if ($stu_id_sql->num_rows > 0) {
  ...
  if (isset($_GET['sub_center_id']) && !empty($_GET['sub_center_id'])) {
    ...
    $pending_count = $conn->query("SELECT Students.ID, Students.First_Name, Students.Middle_Name, Students.Last_Name, Students.Unique_ID, Students.Duration,Admission_Sessions.Name as Admission_Session FROM Students LEFT JOIN Invoices ON Students.ID =Invoices.Student_ID LEFT JOIN Payments ON Invoices.Invoice_No = Payments.Transaction_ID LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID = Admission_Sessions.ID WHERE Payments.Added_By IN ($users) AND Invoices.University_ID = " . $_SESSION['university_id'] . " AND Payments.Status = 0 $sessionQuery ORDER BY Students.ID DESC");
    if ($pending_count->num_rows == 0) {
      $pending_count = $conn->query("SELECT Students.ID, Students.First_Name, Students.Middle_Name, Students.Last_Name, Students.Unique_ID , Students.Duration FROM Students LEFT JOIN Invoices ON Students.ID =Invoices.Student_ID LEFT JOIN Payments ON Invoices.Invoice_No = Payments.Transaction_ID  LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID = Admission_Sessions.ID WHERE Payments.Added_By IN ($users) AND Invoices.University_ID = " . $_SESSION['university_id'] . " AND Payments.Status = 0 $sessionQuery ORDER BY Students.ID DESC");
    }
  }
