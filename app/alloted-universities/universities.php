  ...
  <div class="row">
    ...
    while ($university = $universities->fetch_assoc()) { ?>
      <div class="col-md-6 cursor-pointer" onclick="setSessionUniversity('<?= $university['ID'] ?>', '<?= $university['Name'] ?>', '<?= $university['Logo'] ?>', '<?= $university['Has_Unique_Center'] ?>', '<?= $university['Is_B2C'] ?>', '<?= $university['Is_Vocational'] ?>', '<?= $university['Has_LMS'] ?>');">
        <center>
          <img src="<?= $university['Logo'] ?>" alt="logo" data-src="<?= $university['Logo'] ?>" data-src-retina="<?= $university['Logo'] ?>" class="img-fluid" style="max-width:100%;" width="200px">
          <p class="fw-bold mt-2"><?= $university['Name'] ?></p>
        </center>
      </div>
    <?php }
    ?>
