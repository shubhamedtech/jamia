...
## Fetch records
$result_record = "SELECT `ID`, `Name`, `CanCreateSubCenter`, `Email`,`Created_At`, `Code`, `Status`, `Photo`, CAST(AES_DECRYPT(Password, '60ZpqkOnqn0UQQ2MYTlJ') AS CHAR(50)) password FROM Users WHERE Role = 'Center' $center_query $searchQuery $orderby LIMIT ".$row.",".$rowperpage;
$empRecords = mysqli_query($conn, $result_record);
$data = array();

while ($row = mysqli_fetch_assoc($empRecords)) {
  	$admissions_query = $conn->query("SELECT COUNT(ID) AS Applications FROM Students WHERE Added_For = " . $row['ID']);
    $admissions = mysqli_fetch_assoc($admissions_query);
    $totalamount_query = $conn->query("SELECT SUM(Amount) AS totalamount FROM Wallets WHERE Added_By = " . $row['ID']);
    $totalamount = mysqli_fetch_assoc($totalamount_query);
    $total_amount_value = isset($totalamount['totalamount']) ? $totalamount['totalamount'] : '';
    $data[] = array( 
      "Photo"=> $row['Photo'],
      "Name" => $row['Name'],
      "Email" => $row['Email'],
