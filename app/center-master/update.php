<?php
  if(isset($_POST['name']) && isset($_POST['email']) && isset($_POST['id'])){
    ...
    //$add = $conn->query("UPDATE `Users` SET `Name` = '$name', `Short_Name` = '$short_name', `Contact_Name` = '$contact_person_name', `Email` = '$email', `Mobile` = '$contact', `Alternate_Mobile` = '$alternate_contact', `Address` = '$address', `Pincode` = '$pincode', `City` = '$city', `District` = '$district', `State` = '$state',`Vertical` = '$vertical', $photo WHERE ID = $id");
   $add = $conn->query("UPDATE `Users` 
                    SET 
                        `Name` = '$name', 
