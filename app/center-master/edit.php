<?php 
  if(isset($_GET['id'])){
    ...
    $id = intval($_GET['id']);
    $user = $conn->query("SELECT Name, Short_Name, Contact_Name, Code, Email, Mobile, Alternate_Mobile, Address, Pincode, State, City, District ,Vertical FROM Users WHERE ID = $id");
...
<form role="form" id="form-edit-center-master" action="/app/center-master/update" method="POST" enctype="multipart/form-data">
  <div class="modal-body">
   <div class="row">
      <div id="vertical-div" class="col-md-12">
        <div class="form-group form-group-default required">
          <label>Vertical</label>
          <select class="form-control" name="vertical" id="vertical" onchange="getVarticalID(this.value)">
            <option value="1" <?=$user['Vertical'] == 1 ? 'selected':''?>>Edtech Innovate</option>
            <option value="2" <?= $user['Vertical'] == 2 ? 'selected' : '' ?>> IITS</option>
            <option value="3" <?= $user['Vertical'] == 3 ? 'selected' : '' ?>>International</option>
          </select>
        </div>
      </div>
    </div>
  <div class="row">
    ...
    <div class="row">
      ...
      <div class="col-md-4">
        <div class="form-group form-group-default required">
          <!-- <label class="international">Postal Code</label> -->
          <label class="nationals">Pincode</label>
...
<script>
  ...
  var vertical_id = '<?= $user['Vertical_type']; ?>';
  // alert(vertical_id);
     getVarticalID(vertical_id);
  })
  function getVarticalID(vertical_id){
    if(vertical_id !=3){
      $(".international").show();
      $(".national").hide();
    }else{
...
<script src="/assets/country-select-js-master/build/js/countrySelect.min.js"></script>
  <script>
    // $(document).ready(function() {
    //   var selectedCountry = '<?= isset($user['Country']) ? $user['Country'] : '' ?>';
