...
?>
  ...
  <!-- Modal -->
  <div class="modal-header clearfix justify-content-between mb-4">
    <?php if ($_SESSION['Role'] == 'Administrator') { ?>
      <!-- Back Button -->
      <span class="link text-color cursor-pointer" onclick="allot(<?= $id ?>, 'lg')"><i class="fa fa-arrow-left"></i> Back</span>
  ...
  <form role="form" id="form-allot-center-master" action="/app/center-master/allot<?php print $university_is_vocational == 1 ? '-vocational' : '' ?>" method="POST" enctype="multipart/form-data">
    <div class="modal-body">
      <div class="row justify-content-between">
        <div class="col-md-6">
          <h5 class="text-center fw-bold">Allot <?= $name ?></h5>
        </div>
        <div class="col-md-6">
          <?php if (!empty($alloted_counsellor_id)) { ?>
            <button class="btn-danger btn cursor-pointer btn-sm" onclick="removeAllotment()"><i class="fa fa-trash"></i> Remove</button>
          <?php } ?>
        </div>
      </div>
      <div class="row">
        <div class="col-md-6">
          <div class="form-group form-group-default required">
            ...
            <select class="form-control" name="counsellor" id="counsellor" onchange="getSubCounsellor();">
        ...
        <div class="col-md-6">
          <div class="form-group form-group-default">
            ...
            <select class="form-control" name="sub_counsellor" id="sub_counsellor">
              <option value="">Select</option>
            </select>
          </div>
        </div>
