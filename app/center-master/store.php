...
ini_set('display_errors', 1);
    ...
    if($user_type == 1){      
      if($_SESSION['Role']=='Administrator'){
        $last_inserted_center_code = $conn->query("SELECT Code FROM Users WHERE Role = 'Center' AND Is_Unique = 0  ORDER BY Code DESC LIMIT 1");
        if($last_inserted_center_code->num_rows==0){
          ...
          //echo "oth".$last_inserted_center_code;die;
        }
        $is_unique = 0;
      }else{
    ...
    $add = $conn->query("INSERT INTO `Users`(`Name`, `Short_Name`, `Contact_Name`, `Code`, `Email`, `Mobile`, `Alternate_Mobile`, `Address`, `Pincode`, `City`, `District`, `State`, `Password`, `Photo`, `Role`, `Designation`, `Is_Unique`, `Created_By`, `B2B_Partner`,`Vertical`) VALUES ('$name', '$short_name', '$contact_person_name', '$code', '$email', '$contact', '$alternate_contact', '$address', '$pincode', '$city', '$district', '$state', AES_ENCRYPT('12345','60ZpqkOnqn0UQQ2MYTlJ'), '$filename', 'Center', 'Center', $is_unique, ".$_SESSION['ID'].", '$user_type','$vertical')");
    if($add){
