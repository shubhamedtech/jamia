...
if (isset($_SESSION['filterByCourses'])) {
    $filterByCourse = $_SESSION['filterByCourses'];
}
...
$filterByVertical = "";
if (isset($_SESSION['filterByVertical'])) {
    $filterByVertical = $_SESSION['filterByVertical'];
}

$searchQuery .= $filterByDepartment . $filterQueryUser . $filterByDate . $filterBySubCourse . $filterByCourse . $filterByStatus . $filterByVertical;
...
## Fetch records
$result_record = "SELECT Student_Pendencies.ID as Pendency, Student_Pendencies.Status as Pendency_Status, UPPER(DATE_FORMAT(Students.DOB, '%b %d, %Y')) as DOB, Students.Status,Students.form_status, Students.ID,Students.Mother_Name, Students.Added_For, CONCAT(TRIM(CONCAT(Students.First_Name, ' ', Students.Middle_Name, ' ', Students.Last_Name)), ' (', IF(Students.Unique_ID='' OR Students.Unique_ID IS NULL, RIGHT(CONCAT('000000', Students.ID), 6), Students.Unique_ID), ')') as Unique_ID, CONCAT(Students.First_Name, ' ', Students.Middle_Name, ' ', Students.Last_Name) as First_Name, Students.Father_Name, Students.Enrollment_No, Students.OA_Number, Students.Duration, Students.Step, Students.Process_By_Center, Students.Contact, Students.Payment_Received, Students.Document_Verified, Students.Processed_To_University, Admission_Sessions.`Name` as Adm_Session, Admission_Types.`Name` as Adm_Type, Courses.Name as Short_Name, Student_Documents.`Location`, Students.ID_Card, Students.Admit_Card, Students.Exam, correction_form.correction_data,correction_form.status as correctionFormStatus FROM Students LEFT JOIN Student_Pendencies ON Students.ID = Student_Pendencies.Student_ID AND Student_Pendencies.Status != 1 LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID = Admission_Sessions.ID LEFT JOIN Admission_Types ON Students.Admission_Type_ID = Admission_Types.ID LEFT JOIN Courses ON Students.Course_ID = Courses.ID LEFT JOIN Sub_Courses ON Students.Sub_Course_ID = Sub_Courses.ID LEFT JOIN Student_Documents ON Students.ID = Student_Documents.Student_ID AND Student_Documents.`Type` = 'Photo' LEFT JOIN correction_form on correction_form.students_id=Students.ID WHERE Students.Admission_Session_ID = 69 and Students.University_ID = " . $_SESSION['university_id'] . " $searchQuery $role_query $step_query $session_query $orderby LIMIT " . $row . "," . $rowperpage;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
    ...
    $data[] = array(
        ...
        "Sub_Center_Name" => !empty($sub_centers['Name']) ? $sub_centers['Name'] : '',
        "RM" => $rm['Name'],
        "Status" => $row['Status'],
        "DOB" => $row['DOB'],
        "ID_Card" => $row['ID_Card'],
        ...
        'correctionFormStatus' => $row['correctionFormStatus']
    );
}
