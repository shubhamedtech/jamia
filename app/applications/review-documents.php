...
?>
  ...
  <div class="modal-body">
    <div class="row">
      <?php while ($document = $documents->fetch_assoc()) {
        $uniqid = uniqid(); ?>
        <div class="col-md-2 col-6" id="<?= $uniqid ?>">
          <h6><?= $document['Type'] ?> <?php echo in_array(str_replace(" ", "_", $document['Type']), array_keys($pending)) ? ' <span class="text-danger font-weight-bold">(Re-Uploaded)</span>' : '' ?><?php echo array_key_exists($document['Type'], $heading) && in_array($heading[$document['Type']], array_keys($pending)) ? ' <span class="text-danger font-weight-bold">(Re-Uploaded)</span>' : '' ?></h6>
          <?php $images = explode("|", $document['Location']);
          foreach ($images as $image) {
          ?>
            <img src="<?= $image ?>?v=<?= uniqid() ?>" onclick="viewImage('<?= $uniqid ?>')" style="max-width: 150px; max-height:150px" class="cursor-pointer img-fluid">
          <?php } ?>
        </div>
      <?php } ?>
    </div>
  ...
  <div class="modal-footer clearfix text-end">
    <div class="col-md-4 m-t-10 sm-m-t-10">
      <button aria-label="" type="button" onclick="reportPendency('<?= $student_id ?>')" class="btn btn-danger">
        <i class="ti-alert me-2"></i>
        <span>Mark Pendency</span>
      </button>
      <button aria-label="" type="button" onclick="approveDocuments(<?= $id ?>)" class="btn btn-primary">
