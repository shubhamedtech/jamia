...
if ($_SESSION['Role'] != 'Sub-Center') {
  ...
  array_push($header,'Subject Name');
} else {
  unset($header[4]);
  unset($header[5]);
  unset($header[6]);
...
$searchQuery .= $filterByDepartment . $filterQueryUser . $filterByDate . $filterByCourse . $filterByStatus . $filterByVertical;
...
while ($row = mysqli_fetch_row($empRecords)) {
  ...
  // subject name
  $subQuery = $conn->query("SELECT Student_Id, Subject_id FROM Student_Subjects WHERE Student_Id=$row[0]");
  
  $subjectNames = array();
  
  while ($subArr = mysqli_fetch_row($subQuery)) {
      $subNameQuery = $conn->query("SELECT ID, Name FROM Subjects WHERE ID = $subArr[1]");
      $subNameArr = mysqli_fetch_assoc($subNameQuery);
      $subjectNames[] = $subNameArr['Name'];
  }
  ...
  if ($_SESSION['Role'] != 'Sub-Center') {
    ...
    if ($student_fee->num_rows > 0) {
      ...
      if ($fee_structures->num_rows > 0) {
        while ($fee_structure = $fee_structures->fetch_assoc()) {
          //echo"<pre>"; print_r($fee_structure);
          if ($fee_structure['Sharing'] == 1) {
