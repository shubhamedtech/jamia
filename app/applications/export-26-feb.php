  ...
  if ($_SESSION['Role'] != 'Sub-Center') {
    ...
    if ($student_fee->num_rows > 0) {
      ...
      if($fee_structures->num_rows > 0){
        ...
        // Handle the case where $student_fee is not an array or $fee_structure['ID'] does not exist as an index
        // (you may want to log an error or handle this case according to your application logic)
        array_push($row, 0);
    }
            //array_push($row, $student_fee[$fee_structure['ID']]);
