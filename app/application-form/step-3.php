...
if (isset($_POST['inserted_id'])) {
  ...
  /* Start of new flow */
  $step = $conn->query("SELECT Step, Students.Course_ID, Courses.Short_Name FROM Students LEFT JOIN Courses ON Students.Course_ID = Courses.ID WHERE Students.ID = $inserted_id");
  $step = mysqli_fetch_array($step);
  $courseID = $step['Short_Name'];
  $step = $step['Step'];
  $update = 0;
  /* End of new flow */
  ...
  if($courseID == "10th"){
    $eligibility = ['Other'];
  }else if($courseID == "12th Art" || $courseID == "12th Commerce" || $courseID == "12th Science"){
    $eligibility = ["High School","Other"];
  }else{
  ...
  if(in_array('High School', $eligibility)){
  ...
  if(in_array('Intermediate', $eligibility)){
