...
if (isset($_POST['amount']) && isset($_POST['ids'])) {
  ...
  $ids = explode("|", $ids);
  $center_wallet_amounts = array();
  $center_wallet_amount = 0;
    ...
    if ($_SESSION['Role'] == 'Sub-Center') {
      ...
      if ($check_role_query->num_rows > 0) {
        ...
        if (date("Y-m-d", strtotime($courseArr['Created_At'])) >= "2024-03-30") {
          $center_wallet_amounts[] =  strpos($courseArr['courseName'], 'adeeb-e-mahir') !== false ? 1800 : 1600;
        }
      }
  ...
  if ($_SESSION['Role'] == 'Sub-Center') {
  $center_wallet_amount = array_sum($center_wallet_amounts);
  ...
  if ($add) {
    ...
    if ($add) {
      echo json_encode(['status' => 200, 'message' => 'Application has been processed successfully!']);
    } else {
